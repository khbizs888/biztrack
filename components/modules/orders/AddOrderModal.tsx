...
export default function AddOrderModal({ open, onClose }: Props) {
  ...
  async function lookupPhone() {
    ...
    // Auto-format: prepend 6 if starts with 0x (Malaysian number)
    if (/^0\d/.test(phone)) {
      phone = '6' + phone
  ...
  async function onSubmit(data: OrderFormData) {
    ...
    try {
      ...
      if (receiptUrl && customerId && isReceiptBrand) {
        try { await setCustomerReceiptUrl(customerId, receiptUrl) } catch { /* best-effort */ }
      }
      setReceiptFilePath(null) // uploaded file was used — skip cleanup on close
  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        {brandColor && selectedProject && (
          <div className={cn('inline-flex items-center px-2.5 py-1 rounded-md text-xs font-semibold -mt-1 mb-1', brandColor.bg, brandColor.text)}>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
          ...
          <div className="space-y-1">
            ...
            <Controller name="state" control={control} render={({ field }) => (
              <Select
                value={MALAYSIA_STATES.includes(field.value ?? '') ? (field.value ?? '') : field.value ? 'custom' : ''}
                onValueChange={v => {
                  if (v === 'custom') { setShowCustomState(true); field.onChange('') }
                  else { setShowCustomState(false); field.onChange(v) }
                }}
              >
            ...
            {(showCustomState || (watch('state') && !MALAYSIA_STATES.includes(watch('state') ?? ''))) && (
              <Input placeholder="Enter state / country" {...register('state')} />
            )}
          </div>
              ...
              {receiptUrl ? (
                <div className="relative inline-block">
                  ...
                  <button
                    ...
                    className="absolute -top-1.5 -right-1.5 bg-destructive text-destructive-foreground rounded-full w-4 h-4 text-[10px] flex items-center justify-center leading-none"
                  >
                    ×
                  </button>
                </div>
